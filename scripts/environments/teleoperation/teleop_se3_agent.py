...
parser.add_argument(
    ...
    type=str,
    default="keyboard",
    choices=["keyboard", "so101leader", "bi-so101leader", "xlerobot", "dummy"],
...
parser.add_argument(
    "--debug_env",
    action="store_true",
    help="whether to enable debug mode for environment.",
)
...
def _configure_debug_logging() -> None:
    """Ensure INFO level logs from leisaac/isaaclab modules reach stdout."""
    ...
    # make sure relevant namespaces propagate at INFO
    logging.getLogger("leisaac").setLevel(logging.INFO)
...
def main():
    ...
    if "BiArm" in task_name:
        assert args_cli.teleop_device == "bi-so101leader", (
    ...
    if args_cli.debug_env:
        _configure_debug_logging()
        if hasattr(env_cfg, "enable_debug_logging"):
            env_cfg.enable_debug_logging()
    ...
    elif args_cli.teleop_device == "dummy":
        teleop_interface = DummyTeleop(env)
    # else:
    #     raise ValueError(
